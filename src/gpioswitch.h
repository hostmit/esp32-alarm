...
class Gpioswitch {
  public:
    ...
    Gpioswitch(int gpioNum, bool onValue = true, bool gpioConfigured = false);
    ...
    void off();
    void invert();
    void handle();//should be in loop() for specific instance
    void static handleGlobal(); //should be in loop() to handle all instances
};
std::vector<Gpioswitch *> Gpioswitch::instances;


Gpioswitch::Gpioswitch(int gpioNum, bool onValue, bool gpioConfigured){
    #ifdef DEBUGTOSERIAL
        Serial.println("Gpioswitch: Gpioswitch");
    #endif
    
    if (!gpioConfigured) pinMode(gpioNum, OUTPUT);
...
void Gpioswitch::handle() {
    ...
    if (this->swCount!=-1 && millis()-this->startTime > this->cycleDuration*this->swCount - this->swDelay) { // we completed work
        ...
        return;
    } else if (this->cState!=this->onValue && ((millis()-this->startTime) % (this->cycleDuration) < this->swDuration)) { //set onValue
        #ifdef DEBUGTOSERIAL
        ...
        digitalWrite(this->gpioNum,this->onValue);
        return;
    } else if (this->cState!=!this->onValue &&  ((millis()-this->startTime) % (this->cycleDuration) > this->swDuration)) { //set offValue
        #ifdef DEBUGTOSERIAL
